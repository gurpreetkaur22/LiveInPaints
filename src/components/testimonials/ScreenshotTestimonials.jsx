...
import review5 from "/images/review5.webp";
import review6 from "/images/review6.webp";
import review7 from "/images/review7.webp";
...
const ScreenshotTestimonials = () => {
  ...
  const nextTestimonials = useCallback(() => {
    ...
    setIsTransitioning(true);
    setCurrentIndex(
      (prevIndex) => (prevIndex + 1) % testimonialScreenshots.length
    );
  ...
  useEffect(() => {
    ...
    const preloadImages = () => {
      ...
      const prevIndex =
        (currentIndex - 1 + testimonialScreenshots.length) %
        testimonialScreenshots.length;
    ...
    >
        ...
        <div className="mobile-testimonials-container" ref={mobileGalleryRef}>
          ...
          <div className="mobile-indicators">
            {testimonialScreenshots.map((_, index) => (
              <div
                key={index}
                className={`mobile-indicator-dot ${
                  index === currentIndex ? "active" : ""
                }`}
                onClick={() => {
                  if (!isTransitioning) {
      ...
      {!isMobile && (
        <div className="flex justify-center mt-6 sm:mt-8 space-x-2">
          {Array.from({ length: testimonialScreenshots.length }).map(
            (_, index) => (
              <button
                key={index}
                className={`h-2 sm:h-3 rounded-full transition-all duration-300 ${
                  visibleIndices.includes(index)
                    ? "bg-[#FF5D8F] w-6 sm:w-8"
                    : "bg-gray-300 w-2 sm:w-3 hover:bg-[#FF5D8F]/50"
                }`}
                onClick={() => {
                  if (!isTransitioning) {
                    setIsPaused(true);
